  ...
  const handleGoogleAdminLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/popup-blocked') {
        ...
        setUploadStatus({ message: '로그인 중 오류가 발생했습니다. 잠시 후 다시 시도해 주세요.', type: 'error' });
      }
    } finally {
    ...
    <div className="min-h-screen bg-[#f5f6f8] text-slate-900 font-sans">
      ...
      <header className="sticky top-0 z-50 w-full border-b border-slate-200 bg-white/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          >
            ...
            <div>
              <h1 className="text-lg md:text-xl font-extrabold tracking-normal md:tracking-tight text-[#0a44b8]">망고바나나웨딩</h1>
      ...
      <main className="mx-auto w-full max-w-7xl px-6 py-10">
        {view === 'intro' ? (
          <div className="space-y-20 py-10">
            ...
            <section className="text-center space-y-8 max-w-4xl mx-auto">
              ...
              <motion.h2 
                ...
                className="text-4xl md:text-6xl font-black leading-tight text-slate-900 tracking-tight md:tracking-tighter"
              >
                가장 소중한 날,<br />
            ...
            <section className="bg-slate-900 rounded-[3rem] p-12 md:p-20 text-white overflow-hidden relative">
              ...
              <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                <div className="space-y-8">
                  <h3 className="text-2xl md:text-[34px] font-black leading-tight">
          ...
          {/* Main Form Section - Hidden on mobile, visible on desktop */}
          <div className="hidden lg:block lg:col-span-8 space-y-12">
            ...
            >
              <h2 className="text-3xl md:text-4xl font-black leading-tight tracking-tight mb-4">전문 축의대 서비스 예약</h2>
          ...
          {/* Sidebar */}
          {/* Sidebar / Mobile Booking View */}
          <aside className="lg:col-span-4">
            <div className="sticky top-28 space-y-6">
              {/* Mobile Selection Controls (Visible only on mobile) */}
              <div className="block lg:hidden space-y-6 mb-8">
                <h2 className="text-2xl font-black text-slate-900 mb-6">예약 신청</h2>
                
                {/* 1. Date Selection (Mobile) */}
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-500 ml-1">01. 예식 날짜</label>
                  <input 
                    type="date" 
                    value={selectedDate.toISOString().split('T')[0]}
                    onChange={(e) => setSelectedDate(new Date(e.target.value))}
                    className="w-full rounded-2xl border border-slate-200 bg-white p-4 font-bold text-slate-900 focus:border-[#0a44b8] focus:outline-none"
                  />
                </div>

                {/* 2. Time Selection (Mobile) */}
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-500 ml-1">02. 예약 시간</label>
                  <div className="grid grid-cols-2 gap-2">
                    <select 
                      value={isDirectTime ? 'direct' : `${selectedHour}:${selectedMinute}`}
                      onChange={(e) => {
                      ...
                      className="rounded-2xl border border-slate-200 bg-white p-4 font-bold text-slate-900 focus:border-[#0a44b8] focus:outline-none"
                    >
                    ...
                    {isDirectTime && (
                      <input 
                        type="text"
                        value={customTime}
                        onChange={(e) => setCustomTime(formatTimeInput(e.target.value))}
                        placeholder="예: 12:30"
                        className="rounded-2xl border border-slate-200 bg-white p-4 font-bold text-slate-900 focus:border-[#0a44b8] focus:outline-none"
                      />
                    )}
                ...
                {/* 3. Team Selection (Mobile) */}
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-500 ml-1">03. 팀 구성</label>
                  <div className="grid grid-cols-1 gap-2">
                    {TEAM_OPTIONS.map((team) => (
                      <button
                        key={team.id}
                        ...
                        className={`flex items-center justify-between rounded-2xl border p-4 transition-all ${
                          selectedTeamId === team.id
                            ? 'border-[#0a44b8] bg-[#0a44b8]/5 ring-1 ring-[#0a44b8]'
                            : 'border-slate-200 bg-white hover:border-slate-300'
                        }`}
                      >
                        <div className="flex flex-col items-start">
                          <span className={`text-sm font-bold ${selectedTeamId === team.id ? 'text-[#0a44b8]' : 'text-slate-900'}`}>
                            {team.name}
                          </span>
                          <span className="text-xs text-slate-500">{team.price.toLocaleString()}원</span>
                        </div>
                        {selectedTeamId === team.id && <Check className="h-5 w-5 text-[#0a44b8]" />}
                      </button>
                    ))}
                  </div>
                </div>

                {/* 4. Wedding Hall Selection (Mobile) */}
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-500 ml-1">04. 예식장</label>
                  <div className="relative">
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">
                    ...
                    <input 
                      ...
                      onFocus={() => setShowWeddingHallDropdown(true)}
                      className="w-full rounded-2xl border border-slate-200 bg-white py-4 pl-10 pr-4 font-bold text-slate-900 focus:border-[#0a44b8] focus:outline-none"
                    />
                    {showWeddingHallDropdown && weddingHallSearch.trim().length > 0 && (
                      <div className="absolute left-0 right-0 top-full z-20 mt-2 max-h-60 overflow-y-auto rounded-2xl border border-slate-200 bg-white shadow-xl">
                        {filteredWeddingHalls.map((hall, idx) => (
                          <button
                            key={idx}
                            onClick={() => {
                          ...
                          >
                            <p className="font-bold text-sm text-slate-900">{hall.name}</p>
                            <p className="text-[10px] text-slate-500">{hall.address}</p>
                          </button>
                        ))}
                      </div>
                    )}
                ...
                {/* 5. Settlement Type (Mobile) */}
                <div className="space-y-3">
                  <label className="text-sm font-bold text-slate-500 ml-1">05. 정산 방식</label>
                  <div className="grid grid-cols-1 gap-2">
                    {SETTLEMENT_TYPES.map((type) => (
                      <button
                        key={type.id}
                      ...
                      >
                        <div className="flex flex-col items-start">
                          <span className={`text-sm font-bold ${selectedSettlementType === type.id ? 'text-[#0a44b8]' : 'text-slate-900'}`}>
                            {type.name}
                          </span>
                          <span className="text-xs text-slate-500">{type.description}</span>
                        </div>
                        {selectedSettlementType === type.id && <Check className="h-5 w-5 text-[#0a44b8]" />}
                      </button>
                    ))}
                  </div>
                </div>
